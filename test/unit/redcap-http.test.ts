...
const { RedcapAppConfig } = require('../../src/api/configmodels/RedcapAppConfig');
...
describe('REDCap Effect HTTP client', () => {
  ...
  it('form-encodes project validation without exposing the token in the URL', async () => {
    const config = new RedcapAppConfig(); config.enabled = true; config.connection.url = 'https://redcap.example.edu';
  ...
  it('does not retry ordinary 4xx responses', async () => {
    const config = new RedcapAppConfig(); config.enabled = true; config.connection.url = 'https://redcap.example.edu'; let calls = 0;
  ...
  it('form-encodes project settings JSON', async () => {
    const config = new RedcapAppConfig(); config.enabled = true; config.connection.url = 'https://redcap.example.edu'; let body = '';
